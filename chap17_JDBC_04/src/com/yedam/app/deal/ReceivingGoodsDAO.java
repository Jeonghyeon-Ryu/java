...
public class ReceivingGoodsDAO extends DAO {
	...
	public List<DealInfo> SelectAll(){
		...
		try {
			connect();
			String sql = "SELECT r.deal_date, r.product_id, p.product_name, r.product_amount FROM receiving_goods r JOIN products p ON r.product_id=p.product_id ORDER BY 1,2";
			pstmt = conn.prepareStatement(sql);
